...
你是 issue #$1 在 `MrChancy/startup-leads` 上的 orchestrator。**全程对照 `CLAUDE.local.md` 的规则——pr-review / code-review 会把它当 checklist 用。**
...
2. `gh issue view $1 --json title,body,labels,state` 读 issue。
...
24. `gh issue view $1 --json state` 确认自动关闭（PR body 里 `Closes #$1` 应该触发）。

## 9. Cleanup

25. 列出依赖了 #$1 的 open issue：`gh issue list --search "Blocked by #$1"` 或读它们 body 里的 Blocked by。
26. 对那些**只依赖** #$1 的 issue：`gh issue edit <N> --remove-label blocked`。仍有其它 open blocker 的不动。
